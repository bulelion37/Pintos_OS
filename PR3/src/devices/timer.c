...
{
  ticks++;
  struct list_elem *e = list_begin(&sleeping_list), *end_el = list_end(&sleeping_list);
  while(1)
  {
      if(e == end_el)
	      break;
      ...
      if (temp_thread->sleeping_time <= ticks)
      {
          e = list_remove(e);
  ...
  {
      struct thread *cur_thread = thread_current();
      cur_thread->recent_cpu = int_add_float(1, cur_thread->recent_cpu);
      if (timer_ticks() % TIMER_FREQ == 0)
          update_loadAvg_recentCPU();
      if (timer_ticks() % 4 == 0)
          update_priority();
  }
