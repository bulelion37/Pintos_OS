...
void parsingFilename(char *command, char* fileName);

void parsingFilename(char *command, char* fileName)
{
	strlcpy(fileName, command, strlen(command)+1);
	int i = 0;
	while(1)
	{
		//printf("%c %d\n", fileName[i], i);
...
process_execute (const char *file_name) 
{
  ...
  char command[256];
  parsingFilename(file_name, command);
  ...
  sema_down(&thread_current()->multi_lock);
  ...
  struct list_elem *el;
  struct list* l = &thread_current()->child;

  el = list_begin(l);
  while(el != list_end(l))
  {
	  cur_thread = list_entry(el, struct thread, child_elem);
	  if(cur_thread->exit_status == -1)
...
start_process (void *file_name_)
{
 ...
 success = load (file_name, &if_.eip, &if_.esp);
  ...
  sema_up(&thread_current()->parent->multi_lock);
  if (!success) 
    exit(-1);	
  //thread_exit ();
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  while(el != list_end(l))
  {
	thr = list_entry(el, struct thread, child_elem);
	if(child_tid == thr->tid){
		sema_down(&(thr->wait));
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  char temp[256];
  parsingFilename(file_name, temp);
...
/* Open executable file. */
  ...
  char fn_copy[256];
  char *next;
  char *argv[100], *espArr[100];
  int argNum = 0;
  
  //copy to fn_copy & parse
  ...
  for(int i = argNum-1; i>=0; i--)
  {
	  ...
	  *esp = *esp - argLen;
	  totalLen += argLen;
	  espArr[i] = *esp;
	  strlcpy(*esp, argv[i], argLen);	  
  }
  //word-alignment
  int remainder = totalLen % 4;
  if( remainder != 0)
	  *esp -= (4 - remainder);
